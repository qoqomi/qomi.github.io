...
텔레그램에서 URL을 수신하여 콘텐츠를 자동 분석·요약한 뒤 Supabase DB에 저장합니다.  
...
1. **텔레그램 폴링** — `getUpdates(offset)` 호출, 새 메시지 없으면 즉시 종료
...
9. **텔레그램 완료 응답** — 제목 + 태그 + /archive 링크

## 스킬 호출 순서

| 단계 | 스킬 파일 |
|------|----------|
| 시작 시 | `telegram-poller/scripts/poll_messages.ts` |
...
| 완료 | `telegram-poller/scripts/send_response.ts` |
...
| 단계 | 실패 시 동작 |
|------|------------|
| URL 유효성 | 텔레그램 에러 응답 후 스킵 (재시도 없음) |
| 콘텐츠 수집 | Playwright 폴백 1회 |
| AI 처리 | 최대 2회 재시도 후 스킵 |
| DB 저장 | 1회 재시도 후 텔레그램 알림 |
...
텔레그램 `getUpdates`의 중복 처리 방지를 위해 마지막 처리한 `update_id + 1`을 Supabase `agent_state` 테이블에 저장합니다.
